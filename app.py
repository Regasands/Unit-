            ...
            # МЕНЮ
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    for sprite in field_menu.get_structure_sprites():
                        if sprite.rect.collidepoint(event.pos):
                            # Start
                            if tuple(sprite.rect) == (cell_size * 4, cell_size * 4, cell_size * 2, cell_size * 2):
                                state_engine.menu = False
                                state_engine.game = True
                                game.x_hard = 1
                            # HardLevel
                            elif tuple(sprite.rect) == (cell_size * 4, cell_size * 2, cell_size * 2, cell_size * 2):
                                game.x_hard = 1.5
                                state_engine.menu = False
                                state_engine.game = True
                            # DeletUpgrade
                            elif tuple(sprite.rect) == (cell_size * 8, cell_size * 0, cell_size * 2, cell_size * 2):
                                game.reset_simple()
